...
let lengthOfRectangle:number = 25;
let widthOfRectangle:number = 45;
let areaOfRectangle:number = lengthOfRectangle * widthOfRectangle;
console.log(areaOfRectangle);

// Write a program that takes input and calculates the volume of the cube
// Formula V = a3
function volumeOfTheCube(edge:number){
    return edge ** 3;
};
const cubeVolume:number = volumeOfTheCube(34);
console.log(cubeVolume);

// Write a program that check if a given number is positive negative or zero.
let givenNumber:number = 12;
if(givenNumber<0){
